...
home = os.getenv('HOME')
pid_path = home + '/.local/share/infoget/infoget.pid'
usr_data = home + '/.local/share/infoget/dumps/'
conf_path = home + '/.config/infoget'
path_to_script = os.path.dirname(os.path.realpath(__file__))
src_conf_path = path_to_script + '/conf/'
fail = False

if os.path.isfile(pid_path):
	print(R + '[-]' + C + ' One instance of infoget is already running!' + W)
	with open(pid_path, 'r') as pidfile:
		pid = pidfile.read()
	print(G + '[+]' + C + ' PID : ' + W + str(pid))
	print(G + '[>]' + C + ' If infoget  crashed, execute : ' + W + 'rm {}'.format(pid_path))
	sys.exit()
else:
...
parser = argparse.ArgumentParser(description='infoget - The Web Recon Tool You Will Need ))
...
parser.add_argument('--full', help='infoget', action='store_true')
...
def fetch_meta():
	...
	try:
		rqst = requests.get('https://raw.githubusercontent.com/krishna_7339/infoget/master/metadata.json', timeout=5)
		sc = rqst.status_code
		if sc == 200:
			metadata = rqst.text
...
def banner():
	banner = r'''
__________       _______ _______ _______ ________________
...
\_______//    )_)/      (_______|_______|_______/  )_('''
	print(G + banner + W + '\n')
	print(G + '[>]' + C + ' Created By : ' + W + 'Krishna:SM')
	print(G + '|---> ' + C + 'Twitter : ' + W + twitter_url)
	print(G + '|---> ' + C + 'instagram : ' + W + krishna_7339_)
        print(G + '[>]' + C + ' Thanks to    : ' + W +  + @raghava @mahesh'\n')
